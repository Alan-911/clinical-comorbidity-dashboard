...
# ── 🔥 SURGICAL OVERLAY KILLER — targets true overlay layers only ─────────────
st.markdown("""
<style>
/* Remove ONLY real overlay layers (fixed + z-index), not everything */
div[style*="position: fixed"][style*="z-index"]{display:none!important;}
/* Ensure main app is active */
.stApp,.block-container,section.main{opacity:1!important;filter:none!important;pointer-events:auto!important;}
/* Remove Streamlit stuck status/loading overlay */
[data-testid="stStatusWidget"]{display:none!important;}
[data-testid="stDecoration"]{display:none!important;}
/* Keep decorative layers (anatomical bg image/canvas) non-blocking */
img,canvas{pointer-events:none!important;}
/* 🔥 FORCE DARK TEXT — fixes invisible text when Streamlit dark-mode theme overrides our light theme */
...
/* Animations — fade-in + 3D Y-axis rotation (hologram-style, left↔right) */
@keyframes fadeIn{from{opacity:0;}to{opacity:0.55;}}
@keyframes spinY{from{transform:translate(-38%,-50%) rotateY(0deg);}to{transform:translate(-38%,-50%) rotateY(360deg);}}
...
try:
    ...
    bg_html = f'<img src="data:image/png;base64,{b64}" style="position:fixed;top:50%;left:38%;height:80vh;z-index:0;opacity:0.55;pointer-events:none;transform-origin:center center;transform-style:preserve-3d;animation:fadeIn 0.8s ease forwards,spinY 18s linear infinite;will-change:transform;">'
except: pass
