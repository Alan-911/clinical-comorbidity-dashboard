...
def get_data():
    ...
    if os.path.exists(rp):
        ...
        try:
            ...
            fi = fpgrowth(enc, min_support=0.01, use_colnames=True)
...
if len(f) > 0:
    for i, (_, row) in enumerate(f.nlargest(3,'lift').iterrows()):
        ...
        plan_html += f'''<div class="glass-card" style="padding:15px;margin-bottom:10px;">
            <div class="timeline-item">
                <div style="font-size:10px;color:#94a3b8;font-weight:700;">{14+i}:00</div>
                <div style="font-size:13px;font-weight:700;margin:3px 0;">Review {con[:15]}...</div>
                <div style="font-size:11px;color:#64748b;">{ant[:20]} &rarr; {con[:20]}</div>
            </div>
        </div>'''
else:
    plan_html = '<div class="glass-card" style="padding:15px;"><div class="timeline-item"><div style="font-size:13px;font-weight:700;">Standard Monitoring Plan</div></div></div>'

# Top insight
if len(f) > 0:
    top = f.iloc[0]
    insight_html = f'''<div class="glass-card" style="background:linear-gradient(135deg,#0f172a,#1e293b);color:white;border:none;">
        <div style="font-size:10px;color:#94a3b8;font-weight:700;letter-spacing:1px;">TOP CLINICAL INSIGHT</div>
        <div style="font-size:14px;font-weight:700;margin:8px 0;">{clean_fs(top['antecedents'])} &rarr; {clean_fs(top['consequents'])}</div>
        <div style="font-size:11px;opacity:0.7;">LIFT: {top['lift']:.2f} &bull; CONF: {top['confidence']*100:.1f}%</div>
    </div>'''
else:
    insight_html = '<div class="glass-card" style="background:linear-gradient(135deg,#0f172a,#1e293b);color:white;border:none;"><div style="font-size:11px;color:#94a3b8;">No pattern selected</div></div>'
...
if len(f) > 0:
    for i, (_, row) in enumerate(f.nlargest(5,'lift').iterrows(), 1):
        ...
        bs = mcolors.to_hex(cmap_s(sn))
        bc = mcolors.to_hex(cmap_c(cn))
        sc = "#fff" if sn > 0.5 else "#000"
        cc = "#fff" if cn > 0.5 else "#000"
        matrix_rows += f'<tr><td style="font-weight:700;padding:5px 8px;">{i}</td><td style="padding:5px 8px;font-size:11px;">{clean_fs(row["antecedents"])[:22]} &rarr; {clean_fs(row["consequents"])[:15]}</td><td style="background:{bs};color:{sc};text-align:center;padding:5px 8px;">{row["support"]:.2f}</td><td style="background:{bc};color:{cc};text-align:center;padding:5px 8px;">{row["confidence"]:.2f}</td></tr>'

matrix_html = f'''<table style="width:100%;border-collapse:collapse;font-size:12px;">
    <tr style="background:#f1f5f9;font-weight:700;">
        <th style="padding:5px 8px;">#</th><th style="padding:5px 8px;text-align:left;">Pattern</th>
        <th style="padding:5px 8px;">Supp</th><th style="padding:5px 8px;">Conf</th>
    </tr>
    {matrix_rows}
</table>'''
...
.gc{{background:rgba(255,255,255,0.95);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.8);border-radius:15px;padding:18px;box-shadow:0 10px 30px rgba(0,0,0,0.07);margin-bottom:16px;}}
...
.timeline-item{{margin-bottom:12px;padding-left:14px;border-left:2px solid #e2e8f0;position:relative;}}
.timeline-item::before{{content:'';position:absolute;left:-6px;top:2px;width:10px;height:10px;border-radius:50%;background:#3b82f6;}}
...
.cd-modal{{background:#fff;border-radius:20px;padding:32px;max-width:90%;position:relative;box-shadow:0 30px 80px rgba(0,0,0,0.2);max-height:85vh;overflow-y:auto;}}
.cd-close{{position:absolute;top:14px;right:16px;cursor:pointer;font-size:22px;color:#64748b;background:none;border:none;line-height:1;}}
.cd-appt{{border-radius:0 8px 8px 0;padding:10px 14px;margin-bottom:8px;background:#f8fafc;}}
</style>
{bg_html}
...
<div style="display:grid;grid-template-columns:1fr 1.4fr 1.6fr;gap:24px;position:relative;z-index:2;">

  <!-- COL 1: Care Plan + Cards -->
  <div>
    <h3 style="font-size:15px;font-weight:700;margin:0 0 12px;">Dynamic Care Plan</h3>
    {plan_html}
    <div id="demoCard" class="gc" style="cursor:pointer;text-align:center;padding:14px;">
        <h3 style="margin:0;font-size:14px;">Demographic Comorbidity Patterns</h3>
    </div>
  ...
  <!-- COL 2: anatomical model placeholder -->
  <div></div>

  <!-- COL 3: Analytics -->
  <div>
    <div style="display:flex;gap:10px;margin-bottom:16px;">
        <div class="vc"><div style="font-size:11px;color:#94a3b8;font-weight:600;">Visits</div><div style="font-size:24px;font-weight:700;">2,440</div><div class="ekg" style="background:linear-gradient(90deg,transparent,#3b82f6,transparent);"></div></div>
    ...
    <div class="gc">
        ...
        <div style="margin-bottom:16px;">{matrix_html}</div>
        <div style="display:flex;gap:16px;">
            <div id="svgContainer" style="flex:1.2;height:190px;background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;position:relative;">
                <div style="position:absolute;top:6px;right:6px;display:flex;gap:3px;z-index:10;">
            ...
            <div style="flex:1;font-size:11px;padding-top:8px;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1.4fr;gap:14px;">
        <div class="gc" style="border-top:4px solid #0f172a;" id="patternFormAnchor">
...
<!-- MODALS -->
<div id="apptModal" class="cd-overlay"><div class="cd-modal" style="width:520px;"><button id="closeAppt" class="cd-close">&#10005;</button><h3 style="margin-top:0;">Upcoming Appointments</h3><div class="cd-appt" style="border-left:3px solid #3b82f6;"><b>Oct 24</b> &mdash; Endocrinology Follow-up</div><div class="cd-appt" style="border-left:3px solid #ef4444;"><b>Nov 3</b> &mdash; Cardiology Review</div><div class="cd-appt" style="border-left:3px solid #f59e0b;"><b>Nov 18</b> &mdash; Routine Labs</div></div></div>
<div id="schedModal" class="cd-overlay"><div class="cd-modal" style="width:520px;"><button id="closeSched" class="cd-close">&#10005;</button><h3 style="margin-top:0;">Daily Schedule</h3><div class="cd-appt" style="border-left:3px solid #3b82f6;"><b>08:00</b> &mdash; Morning Vitals</div><div class="cd-appt" style="border-left:3px solid #10b981;"><b>10:30</b> &mdash; Medication Review</div><div class="cd-appt" style="border-left:3px solid #f59e0b;"><b>14:00</b> &mdash; Specialist Consultation</div></div></div>
<div id="labsModal" class="cd-overlay"><div class="cd-modal" style="width:600px;"><button id="closeLabs" class="cd-close">&#10005;</button><h3 style="margin-top:0;">Lab Results</h3><table style="width:100%;border-collapse:collapse;font-size:13px;"><tr style="background:#0f172a;color:white;"><th style="padding:8px;">Test</th><th style="padding:8px;">Result</th><th style="padding:8px;">Status</th></tr><tr style="background:#f8fafc;"><td style="padding:8px;">HbA1c</td><td style="padding:8px;">7.2%</td><td style="padding:8px;color:#f59e0b;">&#9888; Monitor</td></tr><tr><td style="padding:8px;">LDL</td><td style="padding:8px;">112 mg/dL</td><td style="padding:8px;color:#ef4444;">&#10007; High</td></tr><tr style="background:#f8fafc;"><td style="padding:8px;">eGFR</td><td style="padding:8px;">78 mL/min</td><td style="padding:8px;color:#10b981;">&#10003; Normal</td></tr></table></div></div>
<div id="advisoryModal" class="cd-overlay"><div class="cd-modal" style="width:720px;"><button id="closeAdvisory" class="cd-close">&#10005;</button><h2 style="margin-top:0;">Specialist Advisory Board</h2><hr style="border:0;border-top:1px solid #e2e8f0;margin:14px 0;"><div style="display:grid;grid-template-columns:1fr 1fr;gap:18px;"><div style="background:#f8fafc;padding:18px;border-radius:12px;"><h4 style="margin-top:0;">Recommended Specialists</h4><p>&#128104;&#8205;&#9877; Endocrinologist<br>&#129536; Cardiologist<br>&#129537; Pulmonologist</p></div><div style="background:#f8fafc;padding:18px;border-radius:12px;"><h4 style="margin-top:0;">Evidence Summary</h4><p>High Lift Chain detected.<br>Confidence &gt;80%.<br>Multi-specialty review recommended.</p></div></div></div></div>
<div id="demoModal" class="cd-overlay"><div class="cd-modal" style="width:600px;"><button id="closeDemo" class="cd-close">&#10005;</button><h3 style="margin-top:0;">Demographic Comorbidity Insights</h3><table style="width:100%;border-collapse:collapse;font-size:13px;"><tr style="background:#0f172a;color:white;"><th style="padding:8px;">Age Group</th><th style="padding:8px;">Condition</th><th style="padding:8px;">Confidence</th></tr><tr style="background:#f8fafc;"><td style="padding:8px;">Senior (65+)</td><td style="padding:8px;">Heart Disease</td><td style="padding:8px;">82%</td></tr><tr><td style="padding:8px;">Adult (40-64)</td><td style="padding:8px;">Diabetes</td><td style="padding:8px;">75%</td></tr><tr style="background:#f8fafc;"><td style="padding:8px;">Adult (40-64)</td><td style="padding:8px;">Hypertension</td><td style="padding:8px;">71%</td></tr></table></div></div>
...
(function() {{
    ...
    function bind(btnId, modalId, closeId) {{
        var btn=document.getElementById(btnId), modal=document.getElementById(modalId), cls=document.getElementById(closeId);
        if(btn&&modal) btn.addEventListener('click', function(){{ show(modalId); }});
        if(cls&&modal) cls.addEventListener('click', function(){{ hide(modalId); }});
        if(modal) modal.addEventListener('click', function(e){{ if(e.target===modal) hide(modalId); }});
    }}
...
# ── Pattern Selection form (must be Streamlit widget) ────────────────────────
