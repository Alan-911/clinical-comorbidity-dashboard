...
/* Native <dialog> — renders in browser top-layer, escapes every stacking context. */
dialog.cd-mo{background:#fff;border:none;border-radius:20px;padding:32px;width:90%;max-width:720px;box-shadow:0 30px 80px rgba(0,0,0,0.25);max-height:85vh;overflow-y:auto;color:#0f172a;font-family:'Inter',-apple-system,sans-serif;}
dialog.cd-mo::backdrop{background:rgba(15,23,42,0.65);backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px);}
dialog.cd-mo[open]{animation:dlgIn 0.18s ease-out;}
...
<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 36px;background:rgba(255,255,255,0.75);backdrop-filter:blur(15px);-webkit-backdrop-filter:blur(15px);border-radius:100px;box-shadow:0 4px 20px rgba(0,0,0,0.05);margin-bottom:18px;position:relative;z-index:10;">
  ...
  <div style="display:flex;gap:20px;font-size:13px;font-weight:600;color:#64748b;">
    <span onclick="document.getElementById('dashModal').showModal()" style="cursor:pointer;transition:color 0.2s;" onmouseover="this.style.color='#3b82f6'" onmouseout="this.style.color='#64748b'">Dashboard</span>
    <span onclick="document.getElementById('apptModal').showModal()" style="cursor:pointer;transition:color 0.2s;" onmouseover="this.style.color='#3b82f6'" onmouseout="this.style.color='#64748b'">Appointments</span>
    <span onclick="document.getElementById('schedModal').showModal()" style="cursor:pointer;transition:color 0.2s;" onmouseover="this.style.color='#3b82f6'" onmouseout="this.style.color='#64748b'">Schedule</span>
    <span onclick="document.getElementById('labsModal').showModal()" style="cursor:pointer;transition:color 0.2s;" onmouseover="this.style.color='#3b82f6'" onmouseout="this.style.color='#64748b'">Labs</span>
  </div>
  <div onclick="document.getElementById('patientModal').showModal()" style="text-align:right;cursor:pointer;" onmouseover="this.style.opacity='0.7'" onmouseout="this.style.opacity='1'"><div style="font-size:10px;font-weight:800;">PATIENT #2440</div><div style="font-size:9px;color:#3b82f6;">&#9679; CONNECTED</div></div>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1.3fr 1.6fr;gap:22px;position:relative;z-index:2;">
  <div>
    ...
    {plan_html}
    <div onclick="document.getElementById('demoModal').showModal()" class="gc" style="cursor:pointer;text-align:center;padding:14px;" onmouseover="this.style.boxShadow='0 10px 30px rgba(59,130,246,0.15)'" onmouseout="this.style.boxShadow='0 10px 30px rgba(0,0,0,0.07)'"><h3 style="margin:0;font-size:14px;">Demographic Comorbidity Patterns</h3></div>
    <div onclick="document.getElementById('advisoryModal').showModal()" class="gc" style="border-left:4px solid #3b82f6;cursor:pointer;" onmouseover="this.style.boxShadow='0 10px 30px rgba(59,130,246,0.15)'" onmouseout="this.style.boxShadow='0 10px 30px rgba(0,0,0,0.07)'">
...
<!-- MODALS -->
<dialog id="dashModal" class="cd-mo" style="width:680px;" onclick="if(event.target===this)this.close()"><button onclick="this.closest('dialog').close()" class="cd-cl">&#10005;</button>
...
<dialog id="patientModal" class="cd-mo" style="width:560px;" onclick="if(event.target===this)this.close()"><button onclick="this.closest('dialog').close()" class="cd-cl">&#10005;</button>
...
<dialog id="apptModal" class="cd-mo" style="width:520px;" onclick="if(event.target===this)this.close()"><button onclick="this.closest('dialog').close()" class="cd-cl">&#10005;</button><h3 style="margin-top:0;">Upcoming Appointments</h3><div class="ap" style="border-left:3px solid #3b82f6;"><b>Oct 24</b> &mdash; Endocrinology Follow-up</div><div class="ap" style="border-left:3px solid #ef4444;"><b>Nov 3</b> &mdash; Cardiology Review</div><div class="ap" style="border-left:3px solid #f59e0b;"><b>Nov 18</b> &mdash; Routine Labs</div></dialog>
<dialog id="schedModal" class="cd-mo" style="width:520px;" onclick="if(event.target===this)this.close()"><button onclick="this.closest('dialog').close()" class="cd-cl">&#10005;</button><h3 style="margin-top:0;">Daily Schedule</h3><div class="ap" style="border-left:3px solid #3b82f6;"><b>08:00</b> &mdash; Morning Vitals</div><div class="ap" style="border-left:3px solid #10b981;"><b>10:30</b> &mdash; Medication Review</div><div class="ap" style="border-left:3px solid #f59e0b;"><b>14:00</b> &mdash; Specialist Consultation</div></dialog>
<dialog id="labsModal" class="cd-mo" style="width:600px;" onclick="if(event.target===this)this.close()"><button onclick="this.closest('dialog').close()" class="cd-cl">&#10005;</button><h3 style="margin-top:0;">Lab Results</h3><table style="width:100%;border-collapse:collapse;font-size:13px;"><tr style="background:#0f172a;color:white;"><th style="padding:8px;">Test</th><th style="padding:8px;">Result</th><th style="padding:8px;">Status</th></tr><tr style="background:#f8fafc;"><td style="padding:8px;">HbA1c</td><td style="padding:8px;">7.2%</td><td style="padding:8px;color:#f59e0b;">&#9888; Monitor</td></tr><tr><td style="padding:8px;">LDL</td><td style="padding:8px;">112 mg/dL</td><td style="padding:8px;color:#ef4444;">&#10007; High</td></tr><tr style="background:#f8fafc;"><td style="padding:8px;">eGFR</td><td style="padding:8px;">78 mL/min</td><td style="padding:8px;color:#10b981;">&#10003; Normal</td></tr></table></dialog>
<dialog id="advisoryModal" class="cd-mo" style="width:720px;" onclick="if(event.target===this)this.close()"><button onclick="this.closest('dialog').close()" class="cd-cl">&#10005;</button><h2 style="margin-top:0;">Specialist Advisory Board</h2><hr style="border:0;border-top:1px solid #e2e8f0;margin:14px 0;"><div style="display:grid;grid-template-columns:1fr 1fr;gap:18px;"><div style="background:#f8fafc;padding:18px;border-radius:12px;"><h4 style="margin-top:0;">Top Consequent Conditions</h4>{advisory_items_html}</div><div style="background:#f8fafc;padding:18px;border-radius:12px;"><h4 style="margin-top:0;">Evidence Summary</h4><div style="font-size:13px;margin-bottom:8px;">&#128200; Max Lift: <b>{max_lift_val}</b></div><div style="font-size:13px;margin-bottom:8px;">&#127919; Avg Confidence: <b>{avg_conf}%</b></div><div style="font-size:13px;margin-bottom:8px;">&#128196; Matching Rules: <b>{filtered_count}</b></div><div style="margin-top:12px;font-size:12px;color:#64748b;">Multi-specialty review recommended for high-lift chains above {round(max_lift_val*0.8,1)}.</div></div></div></dialog>
<dialog id="demoModal" class="cd-mo" style="width:600px;" onclick="if(event.target===this)this.close()"><button onclick="this.closest('dialog').close()" class="cd-cl">&#10005;</button><h3 style="margin-top:0;">Demographic Comorbidity Insights</h3><table style="width:100%;border-collapse:collapse;font-size:13px;"><tr style="background:#0f172a;color:white;"><th style="padding:8px;">Age Group</th><th style="padding:8px;">Condition</th><th style="padding:8px;">Confidence</th></tr><tr style="background:#f8fafc;"><td style="padding:8px;">Senior (65+)</td><td style="padding:8px;">Heart Disease</td><td style="padding:8px;">82%</td></tr><tr><td style="padding:8px;">Adult (40-64)</td><td style="padding:8px;">Diabetes</td><td style="padding:8px;">75%</td></tr><tr style="background:#f8fafc;"><td style="padding:8px;">Adult (40-64)</td><td style="padding:8px;">Hypertension</td><td style="padding:8px;">71%</td></tr></table></dialog>
<script>
(function vitals(){{
