...
def table(pdf, headers, rows, widths, header_color=NAVY, zebra=BG):
    pdf.set_x(pdf.l_margin)
    ...
    for i, row in enumerate(rows):
        ...
        for w, cell in zip(widths, row):
            pdf.cell(w, 5, str(cell)[:int(w / 1.6)], border=0, fill=fill, align="L")
        pdf.ln(5)
    pdf.ln(1)
...
def build():
    ...
    pdf.add_page()
    h1(pdf, "Results - Itemsets & Rules")

    body(pdf,
         "The tables below are sampled directly from data_processed/ "
         "(frequent_itemsets.csv and association_rules.csv) which the "
         "Streamlit app loads at startup. Full CSVs are versioned in the "
         "repository alongside the Dolt-backed raw data.")

    h2(pdf, "Top Frequent Itemsets (FP-Growth, min_support = 0.01)")
    try:
        import csv
        rows = []
        with open(os.path.join(ROOT, "data_processed",
                               "frequent_itemsets.csv")) as f:
            r = csv.DictReader(f)
            data = sorted(r, key=lambda x: float(x["support"]), reverse=True)
        for d in data[:12]:
            rows.append([
                f"{float(d['support']):.3f}",
                d.get("itemsets_string", "")[:60],
                str(d.get("itemsets_string", "").count(",") + 1),
            ])
        table(pdf,
              ["Support", "Itemset", "Size"],
              rows,
              widths=[22, 140, 28])
    except Exception as e:
        body(pdf, f"(Itemsets table unavailable: {e})")

    h2(pdf, "Top Association Rules (sorted by lift)")
    try:
        rows = []
        with open(os.path.join(ROOT, "data_processed",
                               "association_rules.csv")) as f:
            r = csv.DictReader(f)
            data = sorted(r, key=lambda x: float(x["lift"]), reverse=True)
        for d in data[:14]:
            rows.append([
                d.get("rule_string", "")[:64],
                f"{float(d['support']):.3f}",
                f"{float(d['confidence']):.3f}",
                f"{float(d['lift']):.2f}",
            ])
        table(pdf,
        ...
        body(pdf, f"(Rules table unavailable: {e})")

    body(pdf,
         "Reading the tables: support is the share of patients in which the "
         "itemset/rule occurs; confidence is P(consequent | antecedent); "
         "lift > 1 means antecedent and consequent co-occur more than chance "
         "would predict. The dashboard sorts by lift by default and clamps "
         "confidence to a minimum of 0.5 to suppress weak associations.",
         size=8.5)

    # ──────────────── PAGE 6 — PATTERN VISUALIZATIONS ──────────────────────────
    pdf.add_page()
    h1(pdf, "Pattern Visualizations")

    body(pdf,
         "The three figures below were generated by matplotlib from the same "
         "rule set as the dashboard. They mirror the in-app visual story and "
         "are reproduced in the accompanying Jupyter notebook.")

    viz_dir = os.path.join(ROOT, "visualizations")
    scatter = os.path.join(viz_dir, "rules_scatter.png")
    top_rules = os.path.join(viz_dir, "top_rules_bar.png")
    top_items = os.path.join(viz_dir, "top_itemsets_bar.png")

    if os.path.exists(scatter):
        h2(pdf, "Confidence vs Lift (bubble size = support)")
        pdf.image(scatter, x=pdf.l_margin, w=190)
        pdf.ln(2)
        body(pdf,
             "Top-right corner = clinically actionable rules: high "
             "confidence and high lift. The viridis colour ramp accentuates "
             "lift; bubble area is proportional to support.",
             size=8.5)

    if os.path.exists(top_rules) and os.path.exists(top_items):
        y_split = pdf.get_y()
        h2(pdf, "Top 15 rules by lift   |   Top 15 itemsets by support")
        col_w = 92
        y_img = pdf.get_y()
        pdf.image(top_rules, x=pdf.l_margin, y=y_img, w=col_w)
        pdf.image(top_items, x=pdf.l_margin + col_w + 6, y=y_img, w=col_w)

    # ──────────────── PAGE 7 — DEPLOYMENT, LINKS, CONCLUSION ───────────────────
    pdf.add_page()
